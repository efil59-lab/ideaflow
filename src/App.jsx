...
import { parseChecklist, stripTags } from "./ui/Checklist";
...
function Shell({ user, dark, setDark, look, setLook, th }) {
  ...
  const snapshotNoteTasks = async note => {
    const seen = new Set();
    let n = 0;
    if (note.tagProject && note.tagDone && projects.some(p => p.id === note.tagProject)) {
      await addIdea(uid, { text: stripTags(note.text || ""), title: note.title || "",
        status: "done", projectId: note.tagProject, srcNote: note.id,
        colorIdx: note.colorIdx ?? null }).then(() => { n += 1; }).catch(() => {});
    }
...
function Guide({ onClose, onLog, th }) {
  const steps = [
    { icon: "add", title: "שלב 1 — כתוב פתק", text: 'לחיצה על ➕ פותחת פתק חדש מיד — כותבים, ואפשר לצרף תמונה או הקלטת קול. נשמר לבד תוך כדי כתיבה, בלי לבחור תיקייה ובלי החלטות.' },
    { icon: "check", title: "שלב 2 — סדר בקלות", text: 'צבע לפי נושא, הצמדה לראש הרשימה, תזכורת, ורשימת סימון (בתפריט ⋮) — כל שורה הופכת לפריט עם ריבוע לסימון. הכל בלחיצה.' },
    { icon: "folder", title: "שלב 3 — העבר לפרויקט", text: 'כשפתק הופך למשהו רציני — ⋮ ← "העבר לפרויקט", והוא הופך לרעיון פעיל בתוכו. הפרויקטים הם הספריות שבהן העבודה מתנהלת.' },
  ];
  const features = [
    ...
    { icon: "check", title: "רשימות סימון", text: 'בתפריט ⋮ ← "רשימת סימון": כל שורה הופכת לפריט עם ריבוע. נגיעה בריבוע מסמנת כבוצע ומעבירה קו חוצה, Enter מוסיף פריט, ולחיצה חוזרת מחזירה לטקסט רגיל. ליד כל פריט יש אייקון תווית — בוחרים פרויקט מהרשימה, והפריט מקבל צ׳יפ צבעוני עם שם הפרויקט. המשימה מופיעה גם בתוך הפרויקט, תחת "משימות מפתקים" — סימון V שם או בפתק מעדכן את שניהם, ולחיצה על המשימה פותחת עריכה שלה במקום, בלי לעזוב את הפרויקט (משם אפשר גם לפתוח את הפתק המלא). אם תמחק את הפתק, המשימות שכבר סומנו V נשמרות בתוך הפרויקט כרשומות שלו ולא הולכות לאיבוד. גם פתק רגיל, בלי רשימת סימון, יכול לקבל תווית פרויקט — בתפריט ⋯ של הפתק, "תווית פרויקט". אז הפתק כולו מופיע בפרויקט, V שם מסמן אותו כבוצע, והוא נשאר בלשונית פתקים.' },
    ...
    { icon: "bell", title: "תזכורות", text: 'אייקון הפעמון בסרגל התחתון של עורך הפתק מגדיר תזכורת: בעוד שעה / הערב / מחר או זמן מדויק, כולל חזרה קבועה. ההתראה מגיעה גם כשהאפליקציה סגורה, ואפשר לדחות אותה.' },
    { icon: "folder", title: "העברה לפרויקט", text: 'פתק בודד: ⋮ ← "העבר לפרויקט". כמה פתקים יחד: לחיצה ארוכה מסמנת לבחירה מרובה — ואז אפשר להעביר, לצבוע, להצמיד, לארכב או למחוק את כולם יחד.' },
    { icon: "back", title: "החלקה בין המסכים", text: 'החלקה שמאלה/ימינה עוברת בין פתקים, פרויקטים וחיפוש — כמו מסך בית של הטלפון.' },
    { icon: "folder", title: "פרויקטים", text: 'הספריות שלך. כל פרויקט מרכז רעיונות פעילים, עם סימון בוצע ומעקב התקדמות. כוכב ⭐ מעלה פרויקט לראש, וכרטיס הסטטוס בראש המסך מראה כמה פעיל.' },
    ...
    <Modal onClose={onClose} th={th}>
      ...
      <div style={{ background: th.accentSoft, borderRadius: 12, padding: "12px 14px", marginBottom: 12 }}>
        <p style={{ margin: 0, fontSize: 13.5, color: th.accentText, lineHeight: 1.7, textAlign: "right", fontWeight: 500 }}>
          הרעיון פשוט: <strong>כותבים פתק — וכשרוצים, מעבירים לפרויקט.</strong> כל מחשבה, רשימה,
          תמונה או הקלטה נכנסת לפתקים ברגע שהיא עולה, בלי החלטות. הסידור לפרויקטים קורה אחר-כך, כשנוח לך.
        </p>
      </div>
